...
export function generateSlots(
  ...
  const start = new Date(availStartTime);
  const end = new Date(availEndTime);

  // An availability record belongs to one calendar date; it is not recurring.
  if (!isSameDay(new Date(date), start)) return [];
